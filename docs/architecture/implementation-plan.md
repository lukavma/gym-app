...
**Human-provisioned prerequisites (one-time; agent documents the `az` CLI commands but never fabricates completion):** Azure resource group; Azure Database for PostgreSQL Flexible Server (PostgreSQL 16, Burstable B1ms, 32 GiB, Germany West Central; public access with developer-IP + Azure-services firewall rules; TLS required); App Service plan (Linux B1) + App Service (Node 22, Always On, HTTPS-only, startup command for standalone output); Application Insights resource; GitHub repository + OIDC federated credential for Actions (publish-profile secret as fallback); App Settings (`DATABASE_URL`, `SESSION_SECRET`, `APPLICATIONINSIGHTS_CONNECTION_STRING`) and a GitHub environment secret holding the prod `DATABASE_URL` for the migration step. Anything on this list the agent cannot execute or verify is reported as **pending human verification**, never claimed done.

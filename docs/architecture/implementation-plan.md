...
- Vocabulary: domain constant → 17 leaves + `back` rollup (`kind`, display names incl. "Lower Back (Erectors)", positions, `ROLLUP_MEMBERS = { back: ['lats', 'upper_back'] }`); `seedMuscleGroups` upsert syncs `kind`; `muscle_groups` becomes 18 rows (superseded by ADR-010 Amendment 1 — 18 leaves; `muscle_groups` becomes 19 rows).
...
**Phase 9a — Metrics dashboard v1 (2026-09-06, partial Phase 9, owner-approved per `docs/reviews/metrics-dashboard-architecture-evaluation.md` O-11).** One composed read-only screen (`/metrics`) over five cards — current estimates, training, weekly volume, bodyweight, recovery — plus the smallest possible write path: an athlete-curated, ordered selection of up to five exercises for the estimates card (`dashboard_estimate_selections`, migration `0012`). OD-04 (the charting library) stays explicitly **open**: v1 ships inline SVG only, no chart library decision is made or pre-empted. Four of Phase 9's five deliverables above are deferred past this partial phase: tonnage per session/week, per-muscle volume trend chart (waits on OD-04), recommendation acceptance/modification stats, and history search + filters. The decision point for OD-04 moves to the start of Phase 9b (charts).

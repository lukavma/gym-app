...
**Two disclosed Release-1 seams Release 2 must close (added 2026-09-07, `athletic-measurement-profiles-release-1-review.md` L-12).** Neither is reachable today; both become live only once Release 2 unlocks client emission and the profile/basis selectors.

1. **Client active-session DTO stays narrower than the server's.** The server's own `ActiveSessionSetDto` (`src/server/today/service.ts`) widened `weightKg`/`reps` to `number | null` for display; the client's mirror (`src/sync/types.ts`) is untouched — still `number`-only, with no `distanceM`/`durationS`. Release 2's client-DTO widening must add these before any non-`load_reps` active session can render.
2. **`applySessionExerciseUpsert`'s update path does not re-derive `load_basis` on an `exerciseId` swap.** §10.1/I-14 scope the user-scoped derivation to *insert* only. On update, a payload changing `exerciseId` to a different profile is refused by the mirror FK (`invalid_reference`, fails closed); a same-profile swap succeeds but leaves the slot's `load_basis` at its original, now-possibly-stale value. In Release 1 every exercise is `load_reps`/`unspecified`, so no basis can actually go stale yet — Release 2, which unlocks the basis selector, should extend the insert-time re-derivation to this update path too.

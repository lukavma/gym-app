...
**Not the same count as the Metrics dashboard's "work sets."** The Metrics Training card's "work sets" stat is a separate, broader activity count over **every** measurement profile (isWarmup = false, session not discarded — no profile or `volume_counting` gate); this volume model's "work set" is the narrower, muscle-attributable definition above. **Release 2 (O-6, athletic-measurement-profiles §24.2):** the Training card's caption now spells this distinction out — `Completed workouts only. Warm-up sets not counted. Every exercise type counts as a set.` (`src/ui/metrics/copy.ts:37`) — since Release 2 is the first release in which a non-`load_reps` attempt can exist.
...
1. **RP General** — `classification: 'heuristic'`, `sourceRef: 'docs/input/rp-volume-landmarks.md'`, `isBuiltin: true`, immutable (customize = duplicate). Seeding caveats, documented in the preset description itself:
   ...
   - `forearms`, `lower_back`, `lats`, `upper_back`, `adductors` and `tibialis` (ADR-010 Amendment 1) get no RP landmarks — RP has no row for forearms, erectors, adductors or the shin, and its Back row belongs to the rollup. Landmark-less leaves render their volume with no reference band and no invented range (precedent: `forearms` and `lower_back` have shipped this way since the model was accepted).
...
Out (post-MVP or blocked): volume-based recommendations, MEV/MRV auto-estimation (blocked on evidence — GAP-01), per-session volume pacing, any muscle group beyond vocabulary v2's 18 leaves + `back` rollup (a further split, a second rollup, or a hierarchy needs an ADR amendment — ADR-010), automatic inference of a leaf for unclassified rows, per-leaf landmark invention, as-of historical contribution interpretation.

...
- **Rollup semantics (ADR-010).** Rollup totals are derived in the same pass and never stored. The UI renders the reconciliation line `Back = Lats + Upper Back + Unclassified Back`, hiding the unclassified term when it is zero. There is no grand total across groups, so a set's weight is never summed twice: it lands in exactly one leaf series (or the unclassified term) and appears once more only inside the rollup that *reports* those leaves. Raw sets on a rollup are deduplicated per set; with the seeded partition convention (domain-model §3) the Back raw count equals the pre-v2 merged `back` count exactly.

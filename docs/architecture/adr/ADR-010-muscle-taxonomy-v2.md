...
4. **Reporting is mandatory.** One summary line per run in deployment output: `taxonomy-v2 reconciliation: users=N mapped=M updated=U noop=K conflicts=C customDirectBack=D`. `noop` = seeded row missing or no `back` row (already reconciled, removed, hard-deleted, never applied, or name-collided). `conflicts` = a row on the target leaf already exists — the `back` row is left in place (it shows as Unclassified Back) and reported on its own line with user id, slug, exercise id and target in GitHub Actions `::warning::` format so it appears as a run annotation. `customDirectBack` = user-created exercises still holding a direct `back` row (informational, for the owner's manual reclassification). **Conflicts are not unreachable — that was an earlier, incorrect claim (architecture-review M-1).** Release 1's rules (create rejects rollup slugs; update accepts one only as carry-through of a row that already exists) constrain the row being carried through, not the *other* rows in the same update: during the Release 1 → Release 2 window a user can open a seeded exercise, carry its `back` row through, and add the target leaf as a sibling contribution in the same save — an ordinary two-tap editor action, not a defect. Reconciliation itself and exercise creation can never produce a conflict; a deliberate edit like the one above can. A conflict is also **sticky**: its predicate is never consumed, so every later deploy re-detects the same state and re-emits the same warning until the user resolves it by hand in the editor — this is expected recurrence, not a repeated anomaly. Conflicts are never silent and never fail the deploy, because the resulting state is honest and resolvable in the editor. **The mapped-slug loop above is scoped to the 14 rows in the table below, so it cannot see a direct `back` row hand-added (pre-Release-1) to some *other* seeded exercise — that state would otherwise silently falsify the "no `back` row on a seeded exercise" predicate while the deploy output reports a clean run (architecture-review M-2).** Reconciliation additionally scans every seeded exercise for a direct `back` row outside the 14 mapped ids and reports it as its own count, `seededDirectBackUnmapped`, with its own `::warning::` line per row (user id, slug, exercise id) — printed alongside, not folded into, the summary line above.
...
Rule: vertical pulls and shoulder-extension arcs → `lats`; horizontal rows → `upper_back`; hinge-family isometric mid-back involvement (deadlift, trap-bar deadlift) → `upper_back`, keeping `lats` a clean vertical-pull signal. (`barbell-deadlift` also carries a `traps` secondary, which the `upper_back` target is consistent with — but this does not generalize: `other-trap-bar-deadlift`'s contributions are `glutes`/`quads` primary and `hamstrings`/`back`/`forearms` secondary, with no `traps` row at all, so "matching the `traps` secondary" is not the rationale for its `upper_back` target and must not be read as one for any future hinge entry — architecture-review M-4.) The remaining 78 catalog slugs carry no `back` row and are untouched. This table is binding for the implementation; a row may change only by amending this ADR.

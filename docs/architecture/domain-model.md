...
### MuscleGroup
Seeded reference entity (not a hard-coded enum) so groups can be added later without schema change. Vocabulary v2 (ADR-010, accepted 2026-08-23; Amendment 1, 2026-09-09) — **18 leaves + 1 rollup**:
...
- Leaves (stable text slugs): `chest`, `lats`, `upper_back`, `front_delts`, `side_delts`, `rear_delts`, `traps`, `biceps`, `triceps`, `forearms`, `abs`, `quads`, `hamstrings`, `glutes`, `adductors`, `calves`, `lower_back`, `tibialis` (ADR-010 Amendment 1 — the anterior-shin bucket; `calves` is its antagonist and cannot substitute).

...
| `dumbbell-farmers-carry` (unreferenced) | `measurement_profile = load_distance`, `load_basis = per_hand`, **`volume_counting = 'off'`** | `is_seeded AND measurement_profile = 'load_reps' AND NOT EXISTS session_exercises AND NOT EXISTS exercise_prescriptions` — unreferenced only. **L-6 (2026-09-08):** `volume_counting` is now set to `'off'` alongside the profile/basis fields, so the reconciled row ends byte-identical to a fresh seed of the same catalog entry (`exerciseCatalog.ts` already seeds this entry `volumeCounting: "off"`) instead of retaining the pre-Release-2 `'auto'` default |
...
| `bodyweight-plank` (unreferenced) | `measurement_profile = duration` | same unreferenced predicate as the carry |
| `bodyweight-plank` (referenced) | `volume_counting = 'off'` | `is_seeded AND measurement_profile = 'load_reps' AND volume_counting = 'auto' AND EXISTS session_exercises` — same current-convention predicate as the referenced carry above (H-3), so allowed with history. **L-5 (2026-09-08):** a session-referenced Plank was previously left with no reconcile branch at all (stuck `load_reps`/`unspecified`/`auto` forever); its fabricated reps ("0 kg × 60 reps") now stop counting toward abs volume, for exactly the same reason §11.4 already gives for the referenced carry |
...
- **I-11** Migration `0013` is additive, keeps its defaults, contains no `UPDATE`/`DELETE`, and re-runs idempotently; the seed reconcile is state-predicated, id-keyed, and ships in Release 2, never in Release 1. (L-5/L-6, 2026-09-08: the two added `volume_counting`-only branches — referenced plank, and the `'off'` addition to the unreferenced-carry conversion — do not require an amendment here. Neither reinterprets a historical measurement fact; each changes only `volume_counting`, a gate/switch already covered by the existing "current-convention, allowed with history" framing this invariant and §14.3's referenced-carry row establish. The state-predicated, id-keyed, Release-2-only shape is unchanged.)
...
## 29. Release-2 review correction/owner-decision log (2026-09-08)

Two new owner decisions resolving the asymmetries `docs/reviews/athletic-measurement-profiles-release-2-review.md` §5.5 raised (L-5, L-6) against the otherwise-exact §14.3 seed reconcile. Both are implemented in `src/db/seed/reconcileMeasurementProfiles.ts`.

| Finding | Correction | Sections touched |
| --- | --- | --- |
| L-5 | Owner decision: "a session-referenced seeded Plank that must retain its legacy `load_reps` profile gets `volume_counting = 'off'`." A new reconcile branch added for `bodyweight-plank`, mirroring the existing referenced-carry branch's exact predicate shape (`is_seeded AND measurement_profile = 'load_reps' AND volume_counting = 'auto' AND EXISTS session_exercises`) for plank's slug; mutually exclusive with the existing unreferenced-plank branch by the same EXISTS/NOT EXISTS construction as the farmers-carry pair | §14.3, §18.1 (I-11) |
| L-6 | Owner decision: "an unreferenced Farmer's Carry converted to `load_distance`/`per_hand` also gets `volume_counting = 'off'`, matching a fresh seed." The existing unreferenced-carry conversion branch now additionally sets `volume_counting = 'off'`, so the reconciled row is byte-identical to a fresh seed of the same catalog entry | §14.3, §18.1 (I-11) |

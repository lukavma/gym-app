...
*Items 13–16 rest on the **provisional** registry entries `EVIDENCE-032`…`037` (registry §13; PDF/research-note intake not completed as of 2026-09-06; closure condition in the registry's "Verification pass 3"). They are principles in content but provisional in standing: cite them only as "A13 (provisional)" etc., and do not treat them as equivalent to A1–A12 until the intake closes.*

13. **(Provisional) The relationship between repetitions and %1RM is exercise-specific and individually variable; exercise identity is its dominant moderator, while sex, age, and training status are not clearly moderators.** (`EVIDENCE-032` (provisional), corroborated at the equation level by `EVIDENCE-036` (provisional)) — Moderate–High confidence for exercise-dependence and individual variability; Moderate for the sex/age/training-status null, which is a pooled null over 60–90 % 1RM, not proof of no difference. *(Added 2026-09-05, O-17; marked provisional 2026-09-06.)*

14. **(Provisional) An estimated 1RM carries individual-level error of roughly ±10 % (1 SD) and is roughly three times noisier than a directly measured 1RM (median test–retest CV ≈ 4 %).** (`EVIDENCE-034`, `EVIDENCE-035`, `EVIDENCE-036` (all provisional); `EVIDENCE-032` converted through a repetition equation lands in the same band) — High confidence for the order of magnitude (four independent directions); the transfer from load–velocity estimation (`EVIDENCE-035`) to repetition-based estimation is of magnitude, not mechanism. *(Added 2026-09-05, O-17; marked provisional 2026-09-06.)*

15. **(Provisional) Self-reported proximity to failure is systematically under-predicted by about one repetition on average, its error accumulates roughly eight times faster once a set exceeds 12 repetitions to failure, and its accuracy is not established to differ by training status or body region.** (`EVIDENCE-033` (provisional); consistent with `EVIDENCE-030`'s ±1-rep magnitude and its 1-RIR/3-RIR equivalence) — Moderate–High confidence for direction and the 12-repetition break; this **qualifies A10**: the directional "better near failure" pattern stands, but the best synthesis finds it small, and no experience gradient is confirmed. Every measurement paradigm behind this is a pre-set prediction or an intra-set call-out — the accuracy of a post-set retrospective report has never been measured (GAP-11). *(Added 2026-09-05, O-17; marked provisional 2026-09-06.)*

16. **(Provisional) Maximal strength declines slowly and continuously after training cessation — over months, not weeks — and is retained versus non-training controls for months.** (`EVIDENCE-037` (provisional); `EVIDENCE-025` for the early small cost) — Moderate confidence for the continuous decline (one meta-analysis, abstract only); Low for the retention horizon (two trials, abstract only). The maintenance-dose figures behind "reduced volume with preserved intensity holds strength for a while" come from a **narrative review** and are recorded in `EVIDENCE-037` as **context only**, not as evidence — this principle no longer states them. *(Added 2026-09-05, O-17; reframed and marked provisional 2026-09-06.)*
...
12. **(Rests on provisional evidence) Deriving an estimated 1RM only from an athlete's own sets on one exercise identity, only from sets of at most 12 repetitions to failure, displayed with an explicit uncertainty band of about ±10 %, and never auto-applied or used as a strategy trigger.**
    Built on the **provisional** principles A13–A15 (registry items `EVIDENCE-032`…`036`, PDF/research-note intake pending — see the registry's §13 and "Verification pass 3"): A13 (`EVIDENCE-032`: exercise identity dominates, so a per-exercise within-athlete series is the only unit in which the dominant error is constant and cancels), A14 (`EVIDENCE-034`/`035`/`036`: ≈ ±10 % individual error, three times a measured maximum's), A15 (`EVIDENCE-033`: reps-to-failure prediction error breaks upward above 12 repetitions; `EVIDENCE-036`: equations lose accuracy beyond ~10 repetitions), and existing row 15 of `docs/architecture/evidence-to-design.md` (nothing auto-applies). The *structure* — per-exercise, within-athlete, short-set, banded, advisory — is evidence-supported; the **formula** (Epley or any other), the exact ceiling (12 rather than 10 or 13), the band width, and any load translated from the estimate are conventions. The composition `reps + reported RIR` inside a 1RM equation has never been validated against a measured 1RM (GAP-11). Design detail: ADR-011 and `docs/architecture/evidence-to-design.md` row 20. *(Added 2026-09-05, O-17.)*

13. **(Rests on provisional evidence) Treating any recency window on a strength estimate as a data-freshness rule — "based on the last N days of training" — never as a detraining threshold, and never applying per-day decay weights.**
    Built on: the **provisional** A16 (`EVIDENCE-037` (provisional): decay is continuous, slow, and identifies no threshold at any horizon a product would use; its maintenance-dose figures are narrative context, not evidence) and B6 (`EVIDENCE-025`: post-deload dips are expected). The window lengths themselves (e.g. 90 days) are product conventions with no physiological basis and must be worded as such. *(Added 2026-09-05, O-17.)*

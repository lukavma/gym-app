...
// single in-process instance — so the whole file is skipped when
// DATABASE_URL isn't set (matching this repo's existing convention for
// every other real-Postgres-only check, e.g. tests/e2e/seed.ts).
...
describe.skipIf(!DATABASE_URL)("reconcileContributions concurrency (real PostgreSQL)", () => {
  let editorPool: Pool;
  ...
  beforeAll(async () => {
    editorPool = new Pool({ connectionString: DATABASE_URL });
    ...
    // fail loudly, not flakily, if DATABASE_URL points at a shared database
    // instead of one dedicated to this test file.
    const allUsers = await db(editorPool).select({ id: users.id }).from(users);
    if (allUsers.length !== 1) {
      throw new Error(
        `reconcileContributionsConcurrency expects DATABASE_URL to point at a database with exactly this test's one user, found ${allUsers.length}. Run this file against a dedicated disposable database, not a shared dev database.`,
      );
    }
  });
  ...
  async function concurrentEditorSave(exerciseId: string): Promise<void> {
    const racePool = new Pool({ connectionString: DATABASE_URL });
    try {
